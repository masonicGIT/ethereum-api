...
## Remove existing geth node list

jq 'del(.nodes)' <<<"$CONFIG_FILE_CONTENTS" > $CONFIG_FILE_PATH

## Retrieve the current config file once more

CONFIG_FILE_CONTENTS=`cat $CONFIG_FILE_PATH`

## Set an empty geth node list

jq '. + {nodes: []}' <<<"$CONFIG_FILE_CONTENTS" > $CONFIG_FILE_PATH
...
for (( i = 0; i < COUNT; ++i )); do
  ...
  echo "launching node $i/$COUNT ---> tail-f $CURRENT_DIR/log/$ID.log"
  GETH=$GETH bash ./tools/geth/runGethNode.sh $CURRENT_DIR $ID $*
